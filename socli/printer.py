...
def helpman():
    ...
    options_text = (
        make_header("Stack Overflow command line client:")
        + "\n"
        ...
        + "\n"
        + " "
        + bold("--interactive or -i")
        + " : To search in Stack Overflow"
        ...
        + make_warning("socli --res 2 --query foo bar")
        + ": Displays the second search result of the query"
        + ' "foo bar"'
        + "\n"
        + " "
        + bold("--tag or -t")
        ...
        + "list of all tags."
        + "\n    eg: "
        + make_warning("socli --tag javascript,node.js --query foo bar")
        ...
        + "\n    eg: "
        + make_warning("socli -u")
        + ": Prompts and saves your username. Now you can just run "
        + make_warning("socli -u")
        + " to see "
        + "the stats.\n    "
        + make_warning("socli -u 22656")
        + ": Displays info about user ID 22656"
        + "\n"
        + " "
        + bold("--del or -d")
        + " : Deletes the configuration file generated by "
        + make_warning("socli -u")
        + " command."
        + "\n"
        + " "
        ...
        + "\n"
        " " + bold("--open-url or -o") + " : Opens the given url in socli " + "\n"
        " "
        + bold("--version or -v")
    ...
    options_text = "\n".join(
        [
            "\n".join(
                textwrap.wrap(
                    line,
                    width=int(screen_width) - len(subsequent_indent),
...
def display_results(url, raw_output=False):
    ...
    if raw_output:
        ...
        sys.stdout.write(
            json.dumps(
                {
                    "title": urllib.parse.unquote(question_title),
                    "description": urllib.parse.unquote(question_desc),
                    "stats": urllib.parse.unquote(question_stats),
                    "answers": [urllib.parse.unquote(i) for i in answers],
                    "dup_url": urllib.parse.unquote(dup_url) if dup_url else None,
                }
            )
        )
    else:
