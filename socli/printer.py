...
def helpman():
    ...
    optional_text = {
        ...
        "--interactive or -i":"To search in Stack Overflow and display the matching results. You can chose and browse any of the result interactively",
        "--res or -r":"To select and display a result manually and display its most voted answer.\n\n\033[1;33meg: socli --res 2 --query foo bar: Displays the second search result of the query \'foo bar\'",
        "--tag or -t":"To search a query by tag on Stack Overflow.  Visit http://stackoverflow.com/tags to see the list of all tags.\n\033[1;33meg: socli --tag javascript,node.js --query foo bar: Displays the search result of the query \"foo bar\" in Stack Overflow's javascript and node.js tags.",
        "--new or -n":"Opens the Stack Overflow new questions page in your default browser. You can create a new question using it.",
        "--user or -u":"Displays information about the user provided as the next argument(optional). If no argument is provided it will ask the user to enter a default username. Now the user can run the command without the argument."+"\033[1;33m"+" eg: socli -u: Prompts and saves your username. Now you can just run socli -u to see the stats. socli -u 22656: Displays info about user ID 22656",
