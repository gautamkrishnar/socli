    ...
    try:
        ...
        try:
            ...
            while i < len(tmp):
                ...
                answers = [
                    s.get_text() for s in q_tag.find_all("a", class_="post-tag")
                ][0:]
                    ...
                    if (op > 0) and (op <= i):
                        ...
                        while 1:
                            ...
                            qna = printer.inputs(
                                ...
                                + " to next answer, "
                                + printer.bold("b")
                                + " for previous answer or any other key to exit:"
                            )
                            if qna in ["n", "N"]:
                                try:
                                    answer = tmpsoup.find_all(
                                        "div", class_="js-post-body"
                                    )[cnt + 1].get_text()
                                    printer.print_green("\n\nAnswer:\n")
                                    print("-------\n" + answer + "\n-------\n")
                                    cnt = cnt + 1
                                ...
                                if cnt == 1:
                                    ...
                                    continue
                                answer = tmpsoup.find_all("div", class_="js-post-body")[
                                    cnt - 1
                                ].get_text()
                                printer.print_green("\n\nAnswer:\n")
                                print("-------\n" + answer + "\n-------\n")
...
def socli_browse_interactive(query_tag):
    ...
    class SelectQuestionPage(urwid.WidgetWrap):
        ...
        def __init__(self, questions):
            ...
            self.errorText = tui.UnicodeText.to_unicode(
            ...
            frame = urwid.Frame(
                header=self.header,
                body=urwid.Filler(
                    self.questions_box, height=("relative", 100), valign="top"
                ),
                footer=self.footer,
            )
...
def main():
    ...
    if namespace.open_url:
        ...
        if display_condition:
            ...
            try:
                ...
                printer.print_fail(
                    "The URL specified is returning a 404, please check the url and try again!"
                )
                sys.exit(0)
        if open_in_browser:
    ...
    if namespace.res is not None:  # If --res flag is present
        ...
        if (
            namespace.query != [] or namespace.tag is not None
    ...
    if namespace.browse:
        ...
        namespace.tag or namespace.res or namespace.interactive or namespace.browse
