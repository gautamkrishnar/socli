...
class AnswerText(urwid.WidgetWrap):
    ...
    def prev_ans(self):
        ...
        if self.index < 0:
            self.index = 0
            header_for_display.event('answer-bounds', _("No previous answers."))
        else:
    ...
    def next_ans(self):
        ...
        if self.index > len(self.answers) - 1:
            ...
            header_for_display.event('answer-bounds', _("No more answers."))
        else:
            header_for_display.clear('answer-bounds')
        self.set_answer()
...
def showerror(e):
    if DEBUG == True:
        import traceback
        print(_("Error name: ") + e.__doc__)
        print()
        print(_("Description: ") + str(e))
        print()
        traceback.print_exc()
    else:
        return
...
def socli(query):
    ...
    try:
        ...
        print_fail(_("Please check your internet connectivity..."))
...
def helpman():
    ...
    optionsText = make_header(_("Stack Overflow command line client:")) + '\n' + \
        make_green(_("\n\n\tUsage: socli [ Arguments ] < Search Query >\n\n")) + '\n' + \
        make_header(_("[ Arguments ] (optional):\n")) + '\n' + \
        " " + bold(_("--help or -h")) + _(" : Displays this help") + '\n' + \
        " " + bold(_("--query or -q")) + \
          _(" : If any of the following commands are used then you ") + \
          _("must specify search query after the query argument") + '\n' + \
        " " + bold(_("--interactive or -i")) + _(" : To search in Stack Overflow") + \
                                              _(" and display the matching results. You can chose and ") + \
                                                _("browse any of the result interactively") + '\n' + \
        " " + bold(_("--res or -r")) + \
          _(" : To select and display a result manually and display ") + \
          _("its most voted answer. \n    eg: ") + \
          make_warning(_("socli --res 2 --query foo bar")) + _(": Displays the second search result of the query")  + \
          " \"foo bar\""  + '\n' + \
        " " + bold(_("--tag or -t")) + \
              _(" : To search a query by tag on Stack Overflow.  Visit http://stackoverflow.com/tags to see the ") + \
              _("list of all tags.") + \
        ...
        " " + bold(_("--user or -u")) + \
              _(" : Displays information about the user provided as the next argument(optional). If no argument is provided") + \
              _(" it will ask the user to enter a default username. Now the user can run the command without the argument.") + \
              _("\n    eg: ") + make_warning(_("socli -u")) + _(": Prompts and saves your username. Now you can just run ") + \
              make_warning(_("socli -u")) + _(" to see ") + \
              _("the stats.\n    ") + make_warning(_("socli -u 22656")) + _(": Displays info about user ID 22656") + '\n' + \
        " " + bold(_("--del or -d")) + \
              _(" : Deletes the configuration file generated by ") + make_warning(_("socli -u")) + _(" command.") + '\n' + \
        " " + bold(_("--api or -a")) + \
              _(" : Sets a custom API key for socli") + '\n' + \
        " " + bold(_("--sosearch or -s")) + \
              _(" : SoCLI uses google search by default. Use this option to search Stack Overflow directly.") + '\n' + \
        " " + bold(_("--login or -l")) + \
              _(" : Login to Stack Overflow using your email and password.") + '\n' + \
        " " + bold(_("--logout")) + \
              _(" : Logout of Stack Overflow.")

    helpText = make_header(_("\n\n< Search Query >:")) + '\n' + \
        _("\nQuery to search on Stack Overflow") + '\n' + \
        _("\nIf no commands are specified then socli will search Stack ") + \
              _("Overflow and simply displays the first search result's ") + \
              _("most voted answer.") + '\n' + \
        _("If a command is specified then it will work according to the ") + \
              _("command.") + '\n' + \
        make_header(_("\n\nExamples:\n")) + '\n' + \
        '\t' + make_warning((_("socli for loop in python"))) + '\n' + \
        '\t' + make_warning((_("socli -iq while loop in python"))) + '\n' + \
        _("\n\nSoCLI is an open source project hosted on github. Don't forget to star it if you liked it.\nUse GitHub") + \
              _(" issues to report problems: ") + underline(_("http://github.com/gautamkrishnar/socli"))
...
def get_question_stats_and_answer(url):
    ...
    if len(answers) == 0:
        answers.append(_('No answers for this question ...'))
...
def socli_interactive_windows(query):
    ...
    try:
                ...
                while 1:
                    if (op > 0) and (op <= i):
                        ...
                        while 1:
                            global tmpsoup
                            qna = inputs(
                                _("Type ") + bold("o") + _(" to open in browser, ") + bold("n") + _(" to next answer, ") + bold(
                                    "b") + _(" for previous answer or any other key to exit:"))
                            if qna in ["n", "N"]:
                                try:
                                    ...
                                    print_warning(_(" No more answers found for this question. Exiting..."))
                                    sys.exit(0)
                                continue
                            elif qna in ["b", "B"]:
                                if cnt == 1:
                                    print_warning(_(" You cant go further back. You are on the first answer!"))
                                    continue
                                answer = (tmpsoup.find_all("div", class_="post-text")[cnt - 1].get_text())
                                print_green("\n\n" + _("Answer:") + "\n")
                                print("-------\n" + answer + "\n-------\n")
                        ...
                        op = int(input("\n\n" + _("Wrong option. select the option no to continue:")))
            except Exception as e:
                showerror(e)
                print_warning("\n" + _("Exiting..."))
                sys.exit(0)
        ...
        print_warning("\n\n" + _("Encoding error: Use \"chcp 65001\" command before using socli..."))
        sys.exit(0)
    except requests.exceptions.ConnectionError:
        print_fail(_("Please check your internet connectivity..."))
    except Exception as e:
...
def socl_manusearch(query, rn):
    ...
    if rn < 1:
        print_warning(
            _("Count starts from 1. Use: \"socli -i 2 -q python for loop\" for the 2nd result for the query"))
        sys.exit(0)
    query = urlencode(query)
    try:
        randomheaders()
        ...
        print_warning(_("Encoding error: Use \"chcp 65001\" command before "
                      "using socli..."))
        sys.exit(0)
    except requests.exceptions.ConnectionError:
        print_fail(_("Please check your internet connectivity..."))
...
def userpage(userid):
    ...
    try:
        ...
        print_warning("\n" + _("User ID must be an integer."))
        print(
            "\n" + _("Follow the instructions on this page to get your User ID: http://meta.stackexchange.com/a/111130"))
        exit(1)
    ...
    try:
        ...
        print("\t\t" + _("Total Questions Asked: ") + str(len(userprofile.questions.fetch())))
        print('\t\t        ' + _("Accept rate is:")+' %.2f%%.' % rate)
        #check if the user have answers and questions or no.
        if userprofile.top_answer_tags.fetch():
            print('\n' + _("Most experienced on ") + '%s.' % userprofile.top_answer_tags.fetch()[0].tag_name)
        else:
            print(_("You have 0 answers"))
        if userprofile.top_question_tags.fetch():
            print(_('Most curious about') + ' %s.' % userprofile.top_question_tags.fetch()[0].tag_name)
        else:
            print(_("You have 0 questions"))


        x=input(_('c: check another profile, d: delete this user information and exit, any key: save information and exit ') + '\n')
        if(x=='d'):
            del_datafile()
            print(_('Data files deleted...'))
        ...
        showerror(e)
        if str(e) == _("400 [bad_parameter]: `key` doesn't match a known application"):
            print_warning(_("Wrong API key... Deleting the data file..."))
            del_datafile()
            exit(1)
        elif str(e) in (_("not enough values to unpack (expected 1, got 0)"), _("400 [bad_parameter]: ids")):
            global manual
            if manual == 1:
                print_warning(_("Wrong user ID specified..."))
                helpman()
                exit(1)
            print_warning(_("Wrong user ID... Deleting the data file..."))
            del_datafile()
            exit(1)

        # Reaches here when rate limit exceeds
        print_warning(
            _("Stackoverflow exception. This might be caused due to the rate limiting: http://stackapps.com/questions/3055/is-there-a-limit-of-api-requests"))
        print(_("Use http://stackapps.com/apps/oauth/register to register a new API key."))
        set_api_key()
        exit(1)
...
def set_api_key():
    ...
    """
    global app_data
    api_key = inputs(_("Type an API key to continue: "))
    if len(api_key) > 0:
...
def del_datafile():
    ...
    try:
        ...
        print_warning(_("File not created.... Use socli -u to create a new configuration file."))
        exit(0)
...
def get_stats(soup):
    ...
    try:
        ...
        question_stats = _("Could not load statistics.")
...
def captchacheck(url):
    ...
    """
    if google_search:
        googleErrorDisplayMessage = _("Google thinks you're a bot because you're issuing too many queries too quickly! ") + \
                                    _("Now you'll have to wait about an hour before you're unblocked... :(. Use the -s tag ") + \
                                    _("to search via Stack Overflow instead.")
        ...
        if re.search("\.com/nocaptcha", url): # Searching for stackoverflow captcha
            print_warning(_("StackOverflow captcha check triggered. Please wait a few seconds before trying again."))
            exit(0)
...
def retrieveSavedProfile():
    ...
    try:
        ...
        print_warning(_("Default user not set...") + "\n")
        try:
            # Code to execute when first time user runs socli -u
            app_data['user'] = int(inputs(_("Enter your Stackoverflow User ID: ")))
            save_datafile()
            user = app_data['user']
            print_green("\n" + _("UserID saved...") + "\n")
        except ValueError:
...
def parseArguments(command):
    ...
    """
    parser = argparse.ArgumentParser(description=textwrap.dedent(_('Stack Overflow command line client')), add_help=False)

    #Comment this line out if you want to use argparse's default help function
    parser.add_argument('--help', '-h', action='store_true', help=_('Show this help message and exit'))

    #Flags that return true if present and false if not
    parser.add_argument('--new', '-n', action='store_true', help=textwrap.dedent(_("Opens the stack overflow new questions page in your " \
                                                                "default browser. You can create a new question using it.")))
    parser.add_argument('--interactive', '-i', action='store_true', help=textwrap.dedent(_("To search in Stack Overflow and display the matching results. You can choose and browse any of the results interactively")))
    parser.add_argument('--debug', action='store_true', help=_("Turn debugging mode on"))
    parser.add_argument('--sosearch', '-s', action='store_true', help=_("Searches directly on Stack Overflow instead of using Google"))
    parser.add_argument('--api', '-a', action='store_true', help=_("Sets a custom API key for socli"))
    parser.add_argument('--delete', '-d', action='store_true', help=_("Deletes the configuration file generated by socli -u command"))

    #Accepts 1 argument. Returns None if flag is not present and
    #'STORED_USER' if flag is present, but no argument is supplied
    parser.add_argument('--user', '-u', nargs='?', const='(RANDOM_STRING_CONSTANT)', type=str, help=_("Displays information about the user "
    ...
    parser.add_argument('--tag', '-t', nargs='+', help=_("To search a query by tag on stack overflow."
    ...
    parser.add_argument('--query', '-q', nargs='+', default=[], help=_("If any of the following commands are used then you " \
    ...
    #Accepts 1 argument
    parser.add_argument('--res', '-r', type=int, help=_("To select and display a result manually and display "
...
def main():
    ...
    if namespace.new: #If --new flag is present
        import webbrowser
        print_warning(_("Opening stack overflow in your browser..."))
    ...
    if namespace.delete: #If --delete flag is present
        del_datafile()
        print_warning(_("Data files deleted..."))
        sys.exit(0)
    ...
    if namespace.res != None: #If --res flag is present
        ...
        if namespace.query != [] or namespace.tag != None: #There must either be a tag or a query
            ...
            print_warning(_('You must specify a query or a tag. For example, use: "socli -r 3 -q python for loop" '
        ...
        if namespace.interactive and namespace.query == [] and namespace.tag == None:
            print_warning(_('You must specify a query or a tag. For example, use: "socli -iq python for loop" '
