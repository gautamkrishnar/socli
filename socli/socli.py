...
google_search_url = "https://www.google.com/search?q=site:stackoverflow.com+"
# Used to see whether we are currently displaying a question post
question_post = None
# Not None only if in interactive mode. Displays all the questions found.
question_page = None
...
class UnicodeText(urwid.Text):
    ...
    def __init__(self, text):
        # As we were encoding all text to utf-8 in output before with dispstr,
        # do it automatically for all input
...
class QuestionPage(urwid.WidgetWrap):
    ...
    def __init__(self, data):
        ...
        :param data: tuple of (answers, question_title, question_desc,
                               question_stats, question_url)
        """
    ...
    def makeFrame(self, data):
        """
        Returns a new frame that is formatted correctly with respect to the
        window's dimensions.
        ...
        """
        answers, question_title, question_desc, question_stats, \
            question_url = data
        self.data = data
...
class AnswerText(urwid.WidgetWrap):
    ...
    def set_answer(self):
        """
        We must use a box adapter to get the text to scroll when this widget is
        already in a Pile from the main question page. Scrolling is necessary
        for long answers which are longer than the length of the terminal.
        """
        self.content = (
...
class ScrollableTextBox(urwid.ListBox):
    ...
    def mouse_event(self, size, button, **kwargs):
        SCROLL_WHEEL_UP = 4
        SCROLL_WHEEL_DOWN = 5
...
class QuestionDescription(urwid.WidgetWrap):
    ...
    def set_description(self):
        ...
        We must use a box adapter to get the text to scroll when this widget is
        already in a Pile from the main question page. Scrolling is necessary
        for long questions which are longer than the length of the terminal.
        """
...
def print_header(s):
    print(format_str(s, colorama.Fore.MAGENTA))


def print_blue(s):
    print(format_str(s, colorama.Fore.BLUE))


def print_green(s):
    print(format_str(s, colorama.Fore.GREEN))


def print_warning(s):
    print(format_str(s, colorama.Fore.YELLOW))


def print_fail(s):
    print(format_str(s, colorama.Fore.RED))


def print_white(s):
    print(format_str(s, colorama.Fore.WHITE))


def make_header(s):
    return format_str(s, colorama.Fore.MAGENTA)


def make_blue(s):
    return format_str(s, colorama.Fore.BLUE)


def make_green(s):
    return format_str(s, colorama.Fore.GREEN)


def make_warning(s):
    return format_str(s, colorama.Fore.YELLOW)


def make_fail(s):
    return format_str(s, colorama.Fore.RED)


def make_white(s):
    return format_str(s, colorama.Fore.WHITE)


def bold(s):
    return (format_str(s, bcolors.BOLD))


def underline(s):
    return (format_str(s, bcolors.UNDERLINE))
...
def socli(query):
    ...
    try:
        ...
        showerror(e)
        print_warning(
            "\n\nEncoding error: Use \"chcp 65001\" command before using "
            "socli..."
        )
        sys.exit(0)
...
def helpman():
    ...
    helpText = (
        ...
        "command."
        "\n\n\n" +
        make_header("Examples:") +
        '\n\n'
        '\t' + make_warning("socli for loop in python") +
        '\n\t' + make_warning("socli -iq while loop in python") +
        "\n\n\nSoCLI is an open source project hosted on github. Don't forget "
        "to star it if you liked it.\nUse GitHub issues to report problems: " +
        underline("http://github.com/gautamkrishnar/socli")
    )

    screenHeight, screenWidth = subprocess.check_output(
        ['stty', 'size']).split()
    subsequent_indent = '    '
    optionsText = '\n'.join([
        '\n'.join(
            textwrap.wrap(
                line,
                width=int(screenWidth) - len(subsequent_indent),
...
def get_question_stats_and_answer(url):
    ...
    answers = [
        s.get_text() for s in soup.find_all("div", class_="post-text")
...
def socli_interactive_windows(query):
    """
    Interactive mode basic implimentation for windows, since urwind doesn't
    suports CMD.
    :param query:
    :return:
    """
    try:
        ...
        try:
            ...
            try:
                op = int(inputs(
                    "\nType the option no to continue or any other key to "
                    "exit:"
                ))
                while 1:
                    if (op > 0) and (op <= i):
                        dispres(sourl + question_local_url[op - 1])
                        # this is because the 1st post is the question itself
                        cnt = 1
                        while 1:
                            global tmpsoup
                            qna = inputs(
                                "Type " + bold("o") + " to open in browser, " +
                                bold("n") + " to next answer, " + bold("b") +
                            ...
                            if qna in ["n", "N"]:
                                try:
                                    ...
                                    print_warning(
                                        " No more answers found for this "
                                        "question. Exiting..."
                                    )
                                    sys.exit(0)
                                continue
                                ...
                                print_warning("Opening in your browser...")
                                browser.open(
                                    sourl + question_local_url[op - 1])
                            else:
                                break
                        sys.exit(0)
                    else:
                        op = int(input(
                            "\n\nWrong option. select the option no to "
                            "continue:"
                        ))
            except Exception as e:
                showerror(e)
...
def socli_interactive(query):
    ...
    class SelectQuestionPage(urwid.WidgetWrap):
        ...
        def __init__(self, questions):
            ...
            widgets = [
                self.display_text(i, q) for i, q in enumerate(questions)]
            self.questions_box = ScrollableTextBox(widgets)
            self.header = UnicodeText(
                ('less-important', 'Select a question below:\n')
            )
            self.footerText = '0-' + str(len(self.questions) - 1) + \
                ': select a question, any other key: exit.'
            self.errorText = UnicodeText.to_unicode(
                'Question numbers range from 0-' +
        ...
        def select_question(self, url, index):
            ...
            if self.cachedQuestions[index] is not None:
                ...
                if not google_search:
                    url = sourl + url
                question_title, question_desc, question_stats, \
                    answers = get_question_stats_and_answer(url)
                question_post = QuestionPage(
                    (answers, question_title, question_desc, question_stats,
                     url)
                )
...
def socl_manusearch(query, rn):
    ...
    if rn < 1:
        print_warning(
            "Count starts from 1. Use: \"socli -i 2 -q python for loop\" for "
            "the 2nd result for the query"
        )
        sys.exit(0)
    query = urlencode(query)
    try:
...
def userpage(userid):
    ...
    try:
        ...
        print(
            "\nFollow the instructions on this page to get your User ID: "
            "http://meta.stackexchange.com/a/111130"
        )
        exit(1)
    ...
    try:
        ...
        print_warning(
            "Stackoverflow exception. This might be caused due to the rate "
            "limiting: http://stackapps.com/questions/3055/is-there-a-limit-of"
            "-api-requests"
        )
        print(
            "Use http://stackapps.com/apps/oauth/register to register a new "
            "API key."
        )
        set_api_key()
        exit(1)
...
def del_datafile():
    ...
    try:
        ...
        print_warning(
            "File not created.... Use socli -u to create a new configuration "
            "file."
        )
        exit(0)
...
def loaduseragents():
    ...
    global uas
    uas = []
    with open(os.path.join(os.path.dirname(__file__), "user_agents.txt"),
              'rb') as uaf:
...
def get_stats(soup):
    ...
    question_stats = soup.find_all(
        "span", class_="vote-count-post")[0].get_text()
    try:
        question_stats = (
            "Votes " + question_stats + " | " +
            soup.find_all(
                "div",
                class_="module question-stats")[0].get_text()
            .replace("\n", " ").replace("     ", " | ")
        )
    except IndexError as e:
...
def fixGoogleURL(url):
    ...
    if not bool(re.search(r"/questions/[0-9]+", url)) and \
            not bool(re.search(r"\.com/a/[0-9]", url)):
        return None
    # Resolves rare bug in which this is a prefix
    if url[:17] == "https:///url?url=":
        url = url[17:]
...
def captchacheck(url):
    ...
    """
    if google_search:
        googleErrorDisplayMessage = (
            "Google thinks you're a bot because you're issuing too many "
            "queries too quickly! Now you'll have to wait about an hour "
            "before you're unblocked... :(. Use the -s tag to search via "
        ...
        # Searching for stackoverflow captcha check
        if re.search(r"\.com/nocaptcha", url):
            print_warning(
                "StackOverflow captcha check triggered. Please wait a few "
...
def retrieveSavedProfile():
    ...
    Asks the user to enter a User ID and saves it if a previous file is not
    found.
    ...
    try:
        ...
        del_datafile()
        print_warning(
            "Error in parsing the data file, it will be now deleted. Please "
            "rerun the socli -u command."
        )
        exit(1)
    except FileNotFoundError:
        ...
        try:
            ...
            app_data['user'] = int(
                inputs("Enter your Stackoverflow User ID: ")
            )
            save_datafile()
            user = app_data['user']
            print_green("\nUserID saved...\n")
...
def parseArguments(command):
    ...
    parser = argparse.ArgumentParser(
        description=textwrap.dedent('Stack Overflow command line client'),
        add_help=False)
    ...
    parser.add_argument(
        ...
        help=textwrap.dedent(
            "Opens the stack overflow new questions page in your "
    ...
    parser.add_argument(
        '--debug', action='store_true', help="Turn debugging mode on")
    parser.add_argument(
        '--sosearch', '-s', action='store_true',
        help="Searches directly on Stack Overflow instead of using Google")
    parser.add_argument(
        '--api', '-a', action='store_true',
        help="Sets a custom API key for socli")
    parser.add_argument(
    ...
    parser.add_argument(
        '--user', '-u', nargs='?', const='(RANDOM_STRING_CONSTANT)', type=str,
        help=(
            "Displays information about the user provided as the next "
            "argument(optional). If no argument is provided it will ask the "
            "user to enter a default username. Now the user can run the "
            "command without the argument"
        )
    )
    ...
    parser.add_argument(
        '--tag', '-t', nargs='+',
        help=(
            "To search a query by tag on stack overflow. Visit "
            "http://stackoverflow.com/tags to see the list of all tags."
            "\n"
            "eg:- socli --tag javascript,node.js --query foo bar: Displays "
            "the search result of the query \"foo bar\" in stack overflow's "
            "javascript and node.js tags"
        )
    )
    parser.add_argument(
        '--query', '-q', nargs='+', default=[],
        help=(
            "If any of the following commands are used then you must specify "
            "this option and a query following it."
        )
    )
    ...
    parser.add_argument(
        '--res', '-r', type=int,
        help=(
            "To select and display a result manually and display its most "
            "voted answer. \n   eg:- socli --res 2 --query foo bar: Displays "
            "the second search result of the query \"foo bar\"'s most voted "
            "answer"
        )
    )
...
def main():
    ...
    if namespace.user is not None:  # If --user flag is present
        ...
        user = None
        # If user provided a user ID
        if namespace.user != '(RANDOM_STRING_CONSTANT)':
    ...
    if namespace.res is not None:  # If --res flag is present
        ...
        # There must either be a tag or a query
        if namespace.query != [] or namespace.tag is not None:
            socl_manusearch(query, questionNumber)
        else:
            print_warning(
                'You must specify a query or a tag. For example, use: "socli '
                '-r 3 -q python for loop" to retrieve the third result when '
                'searching about "python for loop". You can also use "socli '
                '-r 3 -t python" to retrieve the third result when searching '
                'for posts with the "python" tag.'
            )
    # If query and tag are not both empty
    elif namespace.query != [] or namespace.tag is not None:
    ...
    # If there are no flags
            ...
            not (namespace.tag or namespace.res or namespace.interactive):
        socli(query)
    else:
        # Help text for interactive mode
        if namespace.interactive and namespace.query == [] and \
                namespace.tag is None:
            print_warning(
                'You must specify a query or a tag. For example, use: "socli '
                '-iq python for loop" to search about "python for loop" in '
                'interactive mode. You can also use "socli -it python" '
